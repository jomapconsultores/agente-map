# Proyectos MAP — aplicación móvil (Android e iOS)

Desarrollado por Marco Antonio Posligua San Martín.

## Qué es esto

Proyectos MAP ya funciona como **PWA**: desde el navegador del teléfono se instala
...
La app abre el despliegue en Coolify, **https://proyectos.pensamiento-libre.org**, que
...
mkdir -p ~/claves
keytool -genkey -v -keystore ~/claves/proyectos.keystore -alias proyectos \
...
Los flujos `.github/workflows/movil-android-proyectos.yml` y
`movil-ios-proyectos.yml` compilan en la nube. Configura en *Settings → Secrets and variables → Actions*:

| Nombre | Tipo | Para qué |
|---|---|---|
| `APP_URL` | variable *(opcional)* | otro destino en vez del de Coolify, que ya viene fijado |
| `ANDROID_KEYSTORE_BASE64` | secreto | `base64 -w0 ~/claves/proyectos.keystore` |
